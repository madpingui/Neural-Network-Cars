...
public class NN
{
    ...
    public Layer[] copyLayers(Layer[] sourceLayers)
    {
        ...
        for (int i = 0; i < sourceLayers.Length; i++)
        {
            ...
            Array.Copy(sourceLayers[i].weightsArray, tmpLayers[i].weightsArray, sourceLayers[i].weightsArray.GetLength(0) * sourceLayers[i].weightsArray.GetLength(1));
            Array.Copy(sourceLayers[i].biasesArray, tmpLayers[i].biasesArray, sourceLayers[i].biasesArray.GetLength(0));
        }
        return tmpLayers;
    }
    ...
    public void MutateLayer(float mutationChance, float mutationAmount)
    {
        for (int i = 0; i < n_neurons; i++)
        {
            for (int j = 0; j < n_inputs; j++)
            {
                ...
                {
                    var random = Random.Range(-mutationAmount, mutationAmount);
                    weightsArray[i, j] += random;
                }
            }
